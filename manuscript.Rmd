...
In the latest version of \textit{Computing Competencies for Undergraduate Data Science Curricula} by ACM Data Science Task Force, accessibility is a recommended content for the data science curricula \cite{danyluk2021computing}.
...
Currently, `BrailleR::VI()` function has a capability to auto-generate alt text for basic R graphics and ggplot objects. 
...
Our definition of sonification refers to a data representation method using stereo sound and various audible patterns. For example, `plot(x = 1:5, y = 1:5)` can be sonified using `sonify::sonify(x = 1:5, y = 1:5)`. 
...
One of the authors in this talk, developed a simple R package, called `tactileR` that can generate a ready-to-emboss pdf file from R graphics and ggplot objects. 
