...
Our definition of sonification refers to a data representation method using stereo sound and various audible patterns. For example, Figure \ref{fig:plot-xy} can be made audible by using `sonify` package [@R-sonify] via `r sonify::sonify(x = 1:5, y = 1:5)`.
...
<!-- ```{r sonify-x-y, echo=TRUE, include=TRUE}
...
```{r tactile-producing-code, echo=TRUE, include=TRUE}
library(palmerpenguins)
library(tactileR)
...
```{r tactile-example, fig.cap="Ready-to-print tactile graph in braille.", fig.alt="Boxplot showing the different body mass between penguins species"}
