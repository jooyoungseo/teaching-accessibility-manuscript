...
Our definition of sonification refers to a data representation method using stereo sound and various audible patterns. For example, Figure \ref{fig:plot-xy} can be made audible by using `sonify` package [@R-sonify] like below (audio file is available at \url{\audio{}}).
...
<!--added eval=FALSE to the chunk below because the output and messages were irrelevant-->

```{r tactile-producing-code, echo=TRUE, include=TRUE, cache=TRUE, eval=FALSE}
